...
import { BarChart3, Bot, ChevronLeft, CircleDollarSign, Gauge, Home, LogOut, MessagesSquare, PhoneCall, Settings, ShieldAlert, ShoppingCart, Swords, Target, TimerReset, Trophy, UsersRound } from "lucide-react";
...
const items = [
  ...
  { label: "Usuários", href: "/admin/users", icon: UsersRound, sub: "login e permissões", adminOnly: true },
...
export function Navigation() {
  ...
  const visibleItems = useMemo(() => {
    return items.filter((item) => !item.adminOnly || profile?.role === "admin");
  }, [profile?.role]);
  ...
  const initials = (profile?.fullName || "KAU")
    .split(" ")
    .filter(Boolean)
    .slice(0, 2)
      ...
      <div className="mt-auto space-y-3">
        <div className="rounded-2xl border border-white/10 bg-white/[.035] p-4 shadow-panel">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br from-amber/40 to-purple/30 text-sm font-black">{initials || "KA"}</div>
            <div className="min-w-0">
              <p className="truncate text-sm font-bold">{profile?.fullName || "Carregando..."}</p>
              <p className="text-xs text-white/50">{profile?.role === "admin" ? "Administradora" : "Vendedora"} · {profile?.sellerDisplayName || "KAU"}</p>
            </div>
          </div>
          <button onClick={handleLogout} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-xs font-bold uppercase text-white/62 transition hover:bg-white/[.08] hover:text-white">
            <LogOut size={14} />
            Sair
          </button>
