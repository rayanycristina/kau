...
export async function GET() {
  ...
  const result = await getSupabaseAdminClient().from("coproducers").select(select).eq("company_id", auth.companyId).order("name");
...
export async function POST(request: Request) {
  ...
  const result = await getSupabaseAdminClient().from("coproducers").insert({ company_id: auth.companyId, name, status: "active", email: text(body.email), phone: text(body.phone), notes: text(body.notes), created_by: auth.user.id }).select(select).single();
...
export async function PATCH(request: Request) {
  ...
  const result = await getSupabaseAdminClient().from("coproducers").update({ name, status, email: text(body.email), phone: text(body.phone), notes: text(body.notes) }).eq("company_id", auth.companyId).eq("id", id).select(select).maybeSingle();
