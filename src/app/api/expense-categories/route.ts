...
export async function GET() {
  ...
  const { data, error } = await getSupabaseAdminClient().from("expense_categories").select("*").eq("company_id", auth.companyId).order("is_system", { ascending: false }).order("name");
...
export async function POST(request: Request) {
  ...
  const { data, error } = await getSupabaseAdminClient().from("expense_categories").insert({ company_id: auth.companyId, name, slug, is_system: false, is_active: true, created_by: auth.user.id }).select("*").single();
...
export async function PATCH(request: Request) {
  ...
  const current = await admin.from("expense_categories").select("*").eq("company_id", auth.companyId).eq("id", id).maybeSingle();
  ...
  const result = await admin.from("expense_categories").update(update).eq("company_id", auth.companyId).eq("id", id).select("*").single();
