...
import { applySellerScope, forceSellerNameForProfile, isAdmin, requireAuth, sellerNameMatches } from "@/lib/auth";
...
export async function POST(request: Request) {
  ...
  const normalized = normalizeSale(scopedBody);
...
export async function PATCH(request: Request) {
  ...
  const body = forceSellerNameForProfile(profile, await request.json().catch(() => ({})));
  ...
  if (!isAdmin(profile)) {
    const { data: existing, error: existingError } = await supabase.from("sales").select("seller_name").eq("id", id).single();
    if (existingError) return NextResponse.json({ error: existingError.message }, { status: 500 });
    if (!sellerNameMatches(profile, existing?.seller_name)) return NextResponse.json({ error: "Sem permissão para editar esta venda." }, { status: 403 });
  }
