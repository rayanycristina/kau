...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await getSupabaseAdminClient().rpc("register_coproducer_participation_payment", { p_obligation_id: id, p_confirmed_amount: Math.round(amount * 100) / 100, p_paid_at: paidAt, p_admin_id: auth.user.id });
